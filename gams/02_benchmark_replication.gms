$TITLE Pakistan CGE 2015-16 (MIMAP Model 0 + EXTER, 2 households): benchmark replication
$ONTEXT
...
* ======================= 7. BENCHMARK REPLICATION TEST =======================
Parameter dev(i,*) '% deviation from benchmark (should all be 0)', maxdev;
dev(i,'X')   = 100*(X.l(i)/X0(i) - 1);
dev(i,'Q')   = 100*(Q.l(i)/Q0(i) - 1);
dev(i,'M')   = 100*(M.l(i)/M0(i) - 1);
dev(i,'E')   = 100*(E.l(i)/E0(i) - 1);
dev(i,'L')   = 100*(LD.l(i)/L0(i) - 1);
dev(i,'PQ')  = 100*(PQ.l(i) - 1);
dev(i,'INV') = 100*(INV.l(i)/I0(i) - 1);
maxdev = smax(i, abs(dev(i,'X')));
maxdev = max(maxdev, smax(i, abs(dev(i,'Q'))), smax(i, abs(dev(i,'PQ'))),
             smax(h, abs(100*(YH.l(h)/YH0(h)-1))), abs(100*(ER.l-1)), abs(100*(W.l-1)));
Display dev, maxdev, WALRAS.l, ER.l, W.l, YH.l, SG.l, TOTSAV.l;
Abort$(maxdev > 1e-3 or abs(WALRAS.l) > 1e-6) "BENCHMARK NOT REPLICATED", dev;
Display "BENCHMARK REPLICATED";
