$TITLE Pakistan CGE 2015-16: Hormuz energy shock under two labour-market closures
$ONTEXT
...
Set s 'scenarios' / base, A30, B71 /
    cl 'labour market closure'
        / FULL  'full employment, flexible wage'
          UNEMP 'fixed real wage, unemployment adjusts' /;
Parameter shock(s) 'energy import price rise' / base 0, A30 0.30, B71 0.71 /;

Parameters
   rep(*,*,s,cl) 'results: % change from base unless noted'
   U0(h)         'base utility index'
   Ut(h)         'utility index';
U0(h) = prod(i, CH0(i,h)**beta(i,h));

Loop((cl,s),
* reset to benchmark start point
   X.l(i)=X0(i); VA.l(i)=VA0(i); LD.l(i)=L0(i); D.l(i)=D0(i); E.l(i)=E0(i); M.l(i)=M0(i);
   Q.l(i)=Q0(i); INV.l(i)=I0(i); CH.l(i,h)=CH0(i,h); YH.l(h)=YH0(h);
   PX.l(i)=1; PVA.l(i)=1; WK.l(i)=1; PD.l(i)=1; PE.l(i)=1; PM.l(i)=1; PQ.l(i)=1;
   W.l=1; UNEMP.l=0; ER.l=1; YG.l=YG0; SG.l=SG0; TOTSAV.l=TOTSAV0; WALRAS.l=0;

* closure switch
   W.lo=1e-6; W.up=+inf; UNEMP.lo=-inf; UNEMP.up=+inf;
   if(sameas(cl,'FULL'),  UNEMP.fx = 0;);
   if(sameas(cl,'UNEMP'), W.fx = 1;);
...
* shock
   ...
   rep('Macro','Real GDP (VA)',s,cl)      = 100*(sum(i, VA.l(i))/sum(i, VA0(i)) - 1);
   rep('Macro','Employment',s,cl)         = 100*(sum(i, LD.l(i))/LS0 - 1);
   rep('Macro','Unemployment (% of LF)',s,cl) = 100*UNEMP.l/LS0;
   rep('Macro','Exchange rate',s,cl)      = 100*(ER.l - 1);
   rep('Macro','Wage',s,cl)               = 100*(W.l - 1);
   rep('Macro','Total investment',s,cl)   = 100*(sum(i, INV.l(i))/sum(i, I0(i)) - 1);
   rep('Macro','Total imports (vol)',s,cl)= 100*(sum(i, M.l(i))/sum(i, M0(i)) - 1);
   rep('Macro','Total exports (vol)',s,cl)= 100*(sum(i, E.l(i))/sum(i, E0(i)) - 1);
   rep('Macro','Govt savings (PKR tn)',s,cl) = SG.l;
   rep('Macro','Energy import bill ($ tn)',s,cl) = pwm('Energy')*M.l('Energy');
   rep('Macro','WALRAS (must be 0)',s,cl) = WALRAS.l;
   rep('Macro','Model status',s,cl)       = PAKCGE.modelstat;
   rep('Output',i,s,cl)      = 100*(X.l(i)/X0(i) - 1);
   rep('Imports',i,s,cl)     = 100*(M.l(i)/M0(i) - 1);
   rep('Price PQ',i,s,cl)    = 100*(PQ.l(i) - 1);
   rep('Labour',i,s,cl)      = 100*(LD.l(i)/L0(i) - 1);
   rep('HH income',h,s,cl)   = 100*(YH.l(h)/YH0(h) - 1);
   rep('HH welfare',h,s,cl)  = 100*(Ut(h)/U0(h) - 1);
);
